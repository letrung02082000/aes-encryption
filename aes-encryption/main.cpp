...
int main(int argc, char* argv[])
{
    ...
    try
    {
        if (encryptModeText.empty())
        {
            std::cout << "Encrypt Mode (enc or dec): ";
            std::cin >> encryptModeText;

            if (Utilities::equalsIgnoreCase(encryptModeText, "enc"))
            {
                encryptMode = true;
            }
            else if (Utilities::equalsIgnoreCase(encryptModeText, "dec"))
            {
                encryptMode = false;
            }
        }
    ...
    uint8_t* initializationVector = nullptr;
